...
class EvidenceSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "n_races_contested": int((~self.races["not_up"]).sum()),
...
class Warehouse:
    ...
    def build_as_of(self, as_of: str | date, election_id: str) -> EvidenceSnapshot:
        ...
        results = self.results[self.results["election_id"] == election_id].copy() if len(self.results) else self.results
        ...
        blob = f"{election_id}|{as_of_d.isoformat()}|{len(polls)}|{polls['poll_id'].astype(str).sum() if len(polls) else ''}"
        ...
        return EvidenceSnapshot(
            ...
            results_known=results_known.reset_index(drop=True) if len(results_known) else results_known,
