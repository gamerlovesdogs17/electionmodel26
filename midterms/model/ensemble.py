...
def predictive_stack_weights(
    oof_draws: dict[str, dict[str, list[float]]],
    truths: dict[str, float],
    *,
    seed: int = 0,
    max_draws: int = 128,
) -> dict[str, float]:
    """Compatibility entrypoint for exact empirical predictive-mixture fitting."""
    from midterms.model.empirical_mixture import fit_predictive_mixture

    return fit_predictive_mixture(oof_draws, truths, seed=seed, max_draws=max_draws)["weights"]
